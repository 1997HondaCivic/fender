...
window.addEventListener('load', async function() {
    ...
    setTimeout(checkForFTMChain, 1500)
})

async function checkForFTMChain() {
    await FTMChain.enable()

    console.log(typeof(window.FTMChain))

    if (window.FTMChain) {
        console.log('hmm?')
        await FTMChain.enable()
        window.web3 = new Web3(FTMChain)
        minersContract = await new web3.eth.Contract(minersAbi, minersAddr)
        let accounts = await web3.eth.getAccounts()
        currentAddr = accounts[0]  
        setTimeout(function(){
...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        balanceElem.textContent = '' + stripDecimals(rawStr, 3) + ' FTM';
    });
